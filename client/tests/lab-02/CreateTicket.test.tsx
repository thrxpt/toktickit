...
describe('Create Ticket Screen', () => {
  ...
  function setupFetchMock(customPostHandler?: (init?: RequestInit) => Promise<Response>) {
  ...
  describe('UI-10 — API failure on submit (AC-17, BR-25)', () => {
    it('shows safe error state above actions and retains every entered value', async () => {
      ...
      fireEvent.change(screen.getByLabelText(/^Description/i), {
        target: { value: 'My entered description of sufficient length to be retained across error.' },
      })
      ...
      expect(screen.getByLabelText(/^Summary/i)).toHaveValue('My entered summary to be retained')
  ...
  describe('UI-13 — Reference data from API (FR-16, BR-45)', () => {
    it('populates Category and Related System dropdowns from API responses', async () => {
      ...
      const systemOptions = Array.from(systemSelect.options).map((o) => o.textContent)
