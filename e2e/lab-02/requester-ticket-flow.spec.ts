...
test.describe("Requester Ticket Flow", () => {
  ...
  test("E2E-01 — Select Requester → create Ticket → find it in My Tickets (AC-08, AC-19)", async ({
    ...
    const ticketLink = page
      .locator("table")
      .locator(`a:has-text("${ticketNumber}")`);
    await expect(ticketLink).toBeVisible();
    await expect(
      page.locator("table").locator(`text=${uniqueSummary}`),
    ).toBeVisible();
  });
  ...
  test("E2E-02 — Switch from Requester A to Requester B (AC-04, AC-20)", async ({
    ...
    const { ticketNumber: ticketNumberA } = await createTicket(page, {
      summary: summaryA,
      description:
        "Private information regarding workstation setup for Requester A.",
    });

    // Verify it is visible for Requester A
    await page.goto("/tickets");
    await expect(
      page.locator("table").locator(`a:has-text("${ticketNumberA}")`),
    ).toBeVisible();
  ...
  test("E2E-03 — Attachment lifecycle in the browser (AC-31, AC-35, AC-36, AC-37)", async ({
    ...
    const { ticketId } = await createTicket(page, {
      ...
      description:
        "Testing upload, download, and soft removal through a real browser.",
    });
    ...
    await expect(removedList.locator('button:has-text("Preview")')).toHaveCount(
      0,
    );
  ...
  test("E2E-05 — Direct navigation to another Requester's Ticket URL (AC-30)", async ({
    ...
    const { ticketId: ticketIdA, ticketNumber: ticketNumberA } =
      await createTicket(page, {
        summary: confidentialSummary,
        description:
          "Super sensitive security credentials and confidential logs.",
      });
    ...
    await expect(
      page.locator(
        "text=This ticket does not exist or you do not have permission to view it.",
      ),
    ).toBeVisible();
