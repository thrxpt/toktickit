...
test.describe("Visual Evidence and State Capture", () => {
  test("E2E-04 — API failure via route interception (AC-17, AC-25)", async ({
    ...
    await expect(
      page.locator(
        "text=Database server is unreachable. Please retry shortly.",
      ),
    ).toBeVisible();
  ...
  test("Capture Create Ticket State Evidence", async ({ page }) => {
    ...
    await page.fill(
      "#summary",
      "Valid ticket summary for submitting state test",
    );
    await page.fill(
      "#description",
    ...
    const submitBtn = page.locator(
      'button[type="submit"]:has-text("Submitting…")',
    );
    ...
    await expect(
      page.locator(
        "text=Unsupported file type. Permitted: JPG, PNG, WEBP, PDF.",
      ),
    ).toBeVisible();
    await page.screenshot({
  ...
  test("Capture My Tickets State Evidence", async ({ page }) => {
    ...
    await page.goto(
      "/tickets?search=NO_TICKETS_MATCH_THIS_UNIQUE_STRING_12345",
    );
    await expect(page.locator("text=No matching tickets")).toBeVisible();
    await expect(
      page.locator('button:has-text("Clear Filters")').first(),
    ).toBeVisible();
    await page.screenshot({
    ...
    await expect(
      page.locator(
        '.card:has(h2:has-text("No tickets yet")) a:has-text("Create Ticket")',
      ),
    ).toBeVisible();
    await page.screenshot({
  ...
  test("Capture Ticket Detail State Evidence", async ({ page }) => {
    ...
    await expect(
      page.locator('h4:has-text("Removed Attachments")'),
    ).toBeVisible();
    await page.screenshot({
